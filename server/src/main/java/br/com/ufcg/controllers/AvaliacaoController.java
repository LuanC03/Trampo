...
public class AvaliacaoController {
	...
	public @ResponseBody ResponseEntity<Response> avaliarUsuario(HttpServletRequest request,
		...
		try {
			...
			response = new Response("O usuário foi avaliado com sucesso!", HttpStatus.OK.value(),
	...
	public @ResponseBody ResponseEntity<Response> getAvaliacaoMedia(@RequestBody String login) {
		...
		try {
			...
			response = new Response("Média das avaliações calculada com sucesso!", HttpStatus.OK.value(),
					mediaAvaliacoes);
			return new ResponseEntity<>(response, HttpStatus.OK);
		} catch (Exception e) {
			response = new Response(e.getMessage(), HttpStatus.BAD_REQUEST.value());
